...
class DviBuilder(PdfBuilder):
	...
	def commands(self):
		...
		dvilatex = ["latex", "--interaction=nonstopmode", "-synctex=1"]
		dvitopdf = ["dvipdf"]
		bibtex = ["bibtex"]
		...
		run = 1
		brun = 0
		yield (dvilatex + [self.base_name], "dvilatex run %d; " % (run, ))
		display_results(run)

		# Check for citations
		# Use search, not match: match looks at the beginning of the string
		# We need to run dvilatex twice after bibtex
		...
		# we may save one dvilatex run
		if "Rerun to get cross-references right." in self.out:
			run = run + 1
			yield (dvilatex + [self.base_name], "dvilatex run %d; " % (run, ))
			display_results(run)

		yield (dvitopdf + ["{}.dvi".format(self.base_name)], "dvi2pdf run 0; ")
		self.display("done.\n")
